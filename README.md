...
| `process` | pids/uids/gids (real & effective), `ngroups`/`getgroups`, `getpriority`/`setpriority`/`nice`, `setpgid`/`setsid`/`getpgid`/`getsid`/`getpgrp`, `kill`/`killpg`, `chdir`/`fchdir`/`getcwd`, `exit_group`, raw `fork`, `execve`/`execveat`, `CStrArray` (`std` feature) |
...
| `time` | `Timespec`, `clock_gettime` (vDSO fast path, no syscall) incl. the `*_COARSE`/`BOOTTIME` clocks, `nanosleep` |
| `umask` | `umask` |
