...
| `fs` | `statx`/`stat`/`lstat`/`fstat`, `faccessat`/`access`, `unlinkat`/`mkdirat`/`renameat2`/`symlinkat`/`readlinkat` (+ `unlink`/`mkdir`/`rename`/… shorthands), `fchmodat`/`chmod`, `fchownat`/`chown`/`lchown`, `utimensat`/`utimens` |
