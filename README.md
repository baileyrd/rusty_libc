...
| `fd` | `read`/`write`(`_all`), `pread`/`pwrite`, `readv`/`writev` (`IoSlice`/`IoSliceMut`), `open`/`openat`, `poll`, `pipe2`, `dup`/`dup2`/`dup3`, `close`/`close_range`, `fcntl`, `memfd_create`, `lseek`, `ftruncate`; `O_*`/`F_*`/`POLL*` constants |
