...
| `process` | pids/uids/gids (real & effective), `ngroups`/`getgroups`, `setpgid`/`setsid`/`getpgid`/`getsid`/`getpgrp`, `kill`/`killpg`, `chdir`/`fchdir`/`getcwd`, `exit_group`, raw `fork`, `execve`/`execveat`, `CStrArray` (`std` feature) |
